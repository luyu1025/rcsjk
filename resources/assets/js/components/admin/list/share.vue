            ...
            >
                ...
                <el-table-column width="120" label="状态">
                    <template slot-scope="scope">
                        <p v-if="scope.row.status==2" class="active-font">已发布</p>
                        <p v-else class="info-font">未发布</p>
                    </template>
                </el-table-column>
                <el-table-column
                        prop="created_at"
                ...
                <el-table-column label="操作" width="220">
                    <template slot-scope="scope">
                        <el-button
                                v-if="user.auth==3"
                                size="mini"
                                @click="handleApply(scope.row)">{{t=scope.row.status==2?'隐藏':'发布'}}</el-button>
                        <el-button
                                size="mini"
        ...
        methods:{
            getList(){
                ...
                this.$ajax.post(window.host+'/api/content/getPosts',req).then((res)=>{
                    ...
                    setTimeout(t.getUserBySore,500)
                },(res)=>{})
            },
            ...
            handleApply(row){
                ...
                this.$confirm('是否'+text+'《'+row.title+'》', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    this.$ajax.post(window.host+'/api/content/changeStatus',{post_id:row.id}).then(
                        (res)=>{
                            if (res.data.err_code==0){
                                t.getList()
                                t.$message({
